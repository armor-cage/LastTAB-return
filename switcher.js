...
async function drawGroupFrames(visibleTabs) {
  try {
    ...
    for (const gid of ids) {
      ...
      if (isColumns) {
        // Sortowanie po kolumnach (X), potem po wysokości (Y)
        rects.sort((a,b) => a.l - b.l || a.t - b.t);
        const COL_TOL = 10; 
        const cols = [];

        // Grupuj recty w kolumny (bucket)
        for (const rc of rects) {
            let bucket = cols.find(c => Math.abs(c.l - rc.l) <= COL_TOL);
            if (!bucket) { bucket = { l: rc.l, rects: [] }; cols.push(bucket); }
        ...
        // Rysuj segment dla każdej kolumny
        for (const col of cols) {
            const L = Math.min(...col.rects.map(x=>x.l));
            const T = Math.min(...col.rects.map(x=>x.t));
            const R = Math.max(...col.rects.map(x=>x.r));
            const B = Math.max(...col.rects.map(x=>x.b));
            
            // Pobierz szerokość pierwszego elementu w tej kolumnie (dla chipa)
            const firstInCol = col.rects.reduce((prev, curr) => (curr.t < prev.t ? curr : prev), col.rects[0]);
            ...
            // Wybierz kandydata na chipa (najbardziej na lewo, potem najwyżej)
            if (!chipCandidate || 
                l < chipCandidate.l || 
                (Math.abs(l - chipCandidate.l) <= COL_TOL && t < chipCandidate.t)) {
                
                const title = (g?.title && g.title.trim()) ? g.title : '(grupa)';
                chipCandidate = { 
                    l, t, w, color, title, 
                    tileW: Math.round(firstInCol.w),
                    parentFrame: frame 
                };
            }
        }

        // Dodaj chip do wybranego segmentu (pierwszego logicznie)
        if (chipCandidate && chipCandidate.parentFrame) {
            const title = chipCandidate.title;
            ...
            try {
              ...
              const targetW = Math.max(0, (chipCandidate.tileW || 0) - 2*inset);
              // Chip pozycjonowany absolutnie względem rodzica (ramki segmentu), więc left/top lokalnie
              
              chip.style.left = (chipCandidate.l + inset) + 'px';
              chip.style.top  = (chipCandidate.t - 10) + 'px'; // -10px wysunięcie w górę
...
render();
  ...
  function marqueeStart(e) {
    ...
    marquee = {
      ...
      baseSelectedTiles: new Set(selectedIds),
      baseSelectedGroups: new Set(selectedGroupIds || []),
      lastPreviewGroups: new Set(),
      mode: {
  ...
  function marqueeUpdate(e) {
    ...
    const useFull = !!(marquee.mode.alt);

    const tileHits = new Set();
    const tilesNow = els.grid.querySelectorAll('.tile');
    ...
    tilesNow.forEach(t => {
      ...
      if (useFull ? full : inter) {
          tileHits.add(Number(t.dataset.id));
      }
    });

    const groupHits = new Set();
    const chipsNow = document.querySelectorAll('.group-chip');
    chipsNow.forEach(ch => {
    ...
    const tileGroupMap = new Map(); 
    tilesNow.forEach(t => {
        const gid = Number(t.dataset.groupId);
        if (gid >= 0) tileGroupMap.set(Number(t.dataset.id), gid);
    });

    const realTileHits = new Set();
    tileHits.forEach(tid => {
        const gid = tileGroupMap.get(tid);
        const isCollapsed = (gid >= 0 && collapsedGroupIds && collapsedGroupIds.has(gid));
        
        if (isCollapsed) {
            groupHits.add(gid);
        } else {
            if (gid >= 0 && groupHits.has(gid)) {
            } else {
                realTileHits.add(tid);
            }
        }
    });
    ...
    if (marquee.mode.ctrl) {
        const baseG = marquee.baseSelectedGroups;
        baseG.forEach(g => { if (!groupHits.has(g)) previewGroups.add(g); });
        groupHits.forEach(g => { if (!baseG.has(g)) previewGroups.add(g); });

        const baseT = marquee.baseSelectedTiles;
        baseT.forEach(t => { if (!realTileHits.has(t)) previewTiles.add(t); });
        realTileHits.forEach(t => { if (!baseT.has(t)) previewTiles.add(t); });

    } else {
        groupHits.forEach(g => previewGroups.add(g));
        realTileHits.forEach(t => previewTiles.add(t));
    }

    const allVisibleGroups = new Set();
    chipsNow.forEach(ch => {
        const gid = Number(ch.getAttribute('data-group-id'));
        if (Number.isFinite(gid)) allVisibleGroups.add(gid);
    });
    previewGroups.forEach(g => allVisibleGroups.add(g));
    marquee.baseSelectedGroups.forEach(g => allVisibleGroups.add(g));

    allVisibleGroups.forEach(gid => {
        const shouldBeSelected = previewGroups.has(gid);
        setGroupSelectionVisualById(gid, shouldBeSelected);
    });

    tilesNow.forEach(t => {
        const tid = Number(t.dataset.id);
        const shouldBeSelected = previewTiles.has(tid);
        setTileSelectionVisualById(tid, shouldBeSelected);
    });
  }

  function restoreBaseVisuals() {
     if (!marquee) return;
     const allKnownGroups = new Set([...marquee.baseSelectedGroups, ...marquee.lastPreviewGroups]);
     document.querySelectorAll('.group-chip').forEach(ch => allKnownGroups.add(Number(ch.dataset.groupId)));
     
     allKnownGroups.forEach(gid => {
         setGroupSelectionVisualById(gid, marquee.baseSelectedGroups.has(gid));
     });

     els.grid.querySelectorAll('.tile').forEach(t => {
         const tid = Number(t.dataset.id);
         setTileSelectionVisualById(tid, marquee.baseSelectedTiles.has(tid));
     });
  }
  ...
  function marqueeEnd(e) {
    ...
    const sxRaw = marquee.startX - dScrollX;
    const syRaw = marquee.startY - dScrollY;
    const x1 = Math.min(sxRaw, e.clientX);
    const y1 = Math.min(syRaw, e.clientY);
    const x2 = Math.max(sxRaw, e.clientX);
    const y2 = Math.max(syRaw, e.clientY);

    if ((x2 - x1) < MARQUEE_MIN_PX && (y2 - y1) < MARQUEE_MIN_PX) {
       clearSelection();
       restoreBaseVisuals();
       els.grid.querySelectorAll('.tile').forEach(t => setTileSelectionVisualById(Number(t.dataset.id), false));
       document.querySelectorAll('.group-chip').forEach(ch => setGroupSelectionVisualById(Number(ch.dataset.groupId), false));
    ...
    els.grid.querySelectorAll('.tile').forEach(t => {
      ...
      const inter = !(r.right < rect.left || r.left > rect.right || r.bottom < rect.top || r.top > rect.bottom);
      const full  = (r.left >= rect.left && r.right <= rect.right && r.top >= rect.top && r.bottom <= rect.bottom);
      if (useFull ? full : inter) tileHits.add(Number(t.dataset.id));
    });
    ...
    document.querySelectorAll('.group-chip').forEach(ch => {
       ...
       if (Number.isFinite(gid)) {
           ...
           const inter = !(r.right < rect.left || r.left > rect.right || r.bottom < rect.top || r.top > rect.bottom);
           const full  = (r.left >= rect.left && r.right <= rect.right && r.top >= rect.top && r.bottom <= rect.bottom);
           if (useFull ? full : inter) groupHits.add(gid);
       }
    });
    ...
    tileHits.forEach(tid => {
        ...
        if (isCollapsed) {
            ...
            if (gid >= 0 && groupHits.has(gid)) { } else { realTileHits.add(tid); }
        }
    });

    if (marquee.mode.ctrl) {
        const baseG = marquee.baseSelectedGroups;
        baseG.forEach(g => { if (groupHits.has(g)) groupHits.delete(g); else groupHits.add(g); });
        
        const baseT = marquee.baseSelectedTiles;
        const finalT = new Set();
        baseT.forEach(t => { if (!realTileHits.has(t)) finalT.add(t); });
        realTileHits.forEach(t => { if (!baseT.has(t)) finalT.add(t); });
        
        selectedGroupIds = new Set(groupHits);
        selectedIds = new Set(finalT);
    } else {
        selectedGroupIds = new Set(groupHits);
        selectedIds = new Set(realTileHits);
    }

    if (selectedGroupIds.size > 0) {
        const tilesInSelectedGroups = new Set();
        tileGroupMap.forEach((gid, tid) => {
            if (selectedGroupIds.has(gid)) tilesInSelectedGroups.add(tid);
        });
        for (const t of tilesInSelectedGroups) {
            selectedIds.delete(t);
        }
    }
    ...
    document.querySelectorAll('.group-chip').forEach(c => allGids.add(Number(c.dataset.groupId)));
    selectedGroupIds.forEach(g => allGids.add(g));
    
    allGids.forEach(gid => setGroupSelectionVisualById(gid, selectedGroupIds.has(gid)));
